...
const MountainVistaParallax = ({ title = '', subtitle = '' }) => {
  ...
  return (
    <section
      className="hero-container w-full h-full absolute inset-0 overflow-hidden pointer-events-none"
    ...
    >
      ...
      {layersData.map(layer => (
        <div
          key={layer.className}
          className={`parallax-layer absolute inset-0 bg-repeat-x bg-bottom ${layer.className} animate-parallax-fg pointer-events-none`}
        />
      ))}

      {/* Hero text */}
      {(title.trim() !== '' || subtitle.trim() !== '') && (
        <div className="hero-content relative flex flex-col items-center justify-center h-full text-center pointer-events-none">
          <h1 className="hero-title text-4xl font-bold text-white mb-2">{title}</h1>
          <p className="hero-subtitle text-lg text-white/80">{subtitle}</p>
        </div>
      )}
    </section>
  );
